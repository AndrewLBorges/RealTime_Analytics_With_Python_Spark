...
id_base_sensor = "S-DSA-MP6-CAP15-02368-"
...
if __name__ == "__main__":
    ...
    for counter in range(0, num_msg):
        ...
        # Gera uma combinação de 3 números randômicos
        ...
        # Se o sensor base não estiver na lista final de sensores, então incluimos
        ...
        print(re.sub(r"[\s+]", "", header_leitura_iot) % (id_sensor, id_equipamento, sensor),
              re.sub(r"[\s+]", "", iotmsg_data_event) % (data_evento),
              re.sub(r"[\s+]", "", iotmsg_formato) % (padrao_leitura),
              re.sub(r"[\s+]", "", iotmsg_dados) % (temperatura))